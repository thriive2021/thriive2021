      ...
      while ( have_rows('button_enabled','option') ) : the_row();
        ...
        if(in_array(get_query_var("therapy")[0], $temparr)){ ?> 
		  ...
		  if(get_query_var('payment_type')){
				...
				?>
					 <a class=" <?php if(($status != "Busy" && $status != "On Call" && $cstatus != "On Chat") || (in_array(get_the_id(),$inhouse_therapist) && $status != "Busy" && $therapy == 'tarot-card-reading')){ echo ' consult_online_link_oc btnCallnow'; } else { echo "busybtn";} ?>" id="consult_online_<?php echo get_the_id(); ?>" data-slug="<?php echo $post->post_name; ?>" data-from_status="<?php echo $from_status; ?>" data-therapy="<?php echo get_query_var("therapy")[0]; ?>"  style="<?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo 'border: 1px solid #27265f ;background-color:#fff;color:#eb0029;';} else { echo "color: #fff;";}?>"><i class="fa fa-phone <?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo "busybtn"; }?>" aria-hidden="true"  style="<?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo "color: red;";} else { echo "color: #fff;";}?>"></i>Call Now</a>
				?>
           ...
           <?php  
			...
			?>
					<a class=" <?php if(($status != "Busy" && $status != "On Call" && $cstatus != "On Chat") || (in_array(get_the_id(),$inhouse_therapist) && $status != "Busy" && $therapy == 'tarot-card-reading') ){ echo ' consult_online_link_oc btnCallnow'; } else { echo "busybtn";} ?>" id="consult_online_<?php echo get_the_id(); ?>" data-slug="<?php echo $post->post_name; ?>" data-from_status="<?php echo $from_status; ?>" data-therapy="<?php echo get_query_var("therapy")[0]; ?>" data-remaining="0" style="<?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo 'border: 1px solid #27265f;background-color:#fff;color:#eb0029;';} else { echo "color: #fff;";}?>"><i class="fa fa-phone <?php if(($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") ) { echo "busybtn"; }?>" aria-hidden="true"  style="<?php if(($status == "Busy" || $status == "On Call" || $cstatus == "On Chat")) { echo "color: red;";} else { echo "color: #fff;";}?>"></i>Call Now </a>
			<?php
				//if($inhouse == 1){
			?>	
																								 
					<input type="hidden" id="chat_event" value="<?php echo $event; ?>">
					<a class="btnChatnow <?php if(($status != "Busy" && $status != "On Call" && $cstatus != "On Chat" && $inhouse == 1) || (in_array(get_the_id(),$inhouse_therapist) && $status != "Busy" && $therapy == 'tarot-card-reading')) {  echo "start_chat_oc";}  else { echo "busybtn";}?>" data-img="<?php echo $url; ?>" data-fromuserid="<?php echo $seeker_id; ?>" data-touserid="<?php echo $therapist_id; ?>" data-tousername="<?php echo $name; ?>" data-from_status="<?php echo $from_status; ?>"   data-to_status = "<?php echo $to_status; ?>" data-mobile="" data-msg="<?php echo $msg; ?>" data-slug="<?php echo $post->post_name; ?>" data-therapy="<?php echo get_query_var("therapy")[0]; ?>" data-email="<?php echo $therapist_email; ?>"  data-role="<?php echo $role1; ?>" style="<?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat" || ($inhouse == 1 && $status != "Available")) {  echo 'border: 1px solid #27265f ;background-color:#fff;color:#eb0029;';}  else { if($inhouse != 1){ echo 'border: 1px solid #27265f ;background-color:lightgrey;color:grey;'; } } ?>"><i class="fa fa-comments-o <?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") {  echo "busybtn"; }?>" aria-hidden="true" style="<?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat" ) { echo "color: red;";  } else { echo "color: #27265f;";}?>"></i>Chat Now </a>
            <?php   
