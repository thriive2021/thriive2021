      ...
      while ( have_rows('button_enabled','option') ) : the_row();
        ...
        if(in_array(get_query_var("therapy")[0], $temparr)){ ?> 
		  ...
		  if(get_query_var('payment_type')){
				...
				?>
					 <a class="<?php if($status != "Busy" && $cstatus != "Busy"){ echo ' consult_online_link_oc btnCallnow'; } else { echo "busybtn";} ?>" id="consult_online_<?php echo get_the_id(); ?>" data-slug="<?php echo $post->post_name; ?>" data-from_status="<?php echo $from_status; ?>" data-therapy="<?php echo get_query_var("therapy")[0]; ?>" style="<?php if($status == "Busy" || $cstatus == "Busy") { echo 'border: 1px solid #eb0029 ;background-color:#fff;color:#eb0029;';} else { echo "color: #fff;";}?>"><i class="fa fa-phone <?php if($status == "Busy" || $cstatus == "Busy") { echo "busybtn"; }?>" aria-hidden="true" style="<?php if($status == "Busy" || $cstatus == "Busy") { echo "color: red;";} else { echo "color: #fff;";}?>"></i>Call Now</a>
				?>
           ...
           <?php  
			...
			?>
					<a class="<?php if($status != "Busy" && $cstatus != "Busy"){ echo ' consult_online_link_oc btnCallnow'; } else { echo "busybtn";} ?>" id="consult_online_<?php echo get_the_id(); ?>" data-slug="<?php echo $post->post_name; ?>" data-from_status="<?php echo $from_status; ?>" data-therapy="<?php echo get_query_var("therapy")[0]; ?>" data-remaining="0" style="<?php if($status == "Busy" || $cstatus == "Busy") { echo 'border: 1px solid #eb0029 ;background-color:#fff;color:#eb0029;';} else { echo "color: #fff;";}?>"><i class="fa fa-phone" aria-hidden="true" style="<?php if($status == "Busy" || $cstatus == "Busy") { echo "color: red;";} else { echo "color: #fff;";}?>"></i>Call Now </a>
				   
																								 
					<input type="hidden" id="chat_event" value="<?php echo $event; ?>">
					<a class="btnChatnow <?php if($status != "Busy" && $cstatus != "Busy") {  echo "start_chat_oc";}  else { echo "busybtn";}?>" data-img="<?php echo $url; ?>" data-fromuserid="<?php echo $seeker_id; ?>" data-touserid="<?php echo $therapist_id; ?>" data-tousername="<?php echo $name; ?>" data-from_status="<?php echo $from_status; ?>"   data-to_status = "<?php echo $to_status; ?>" data-mobile="" data-msg="<?php echo $msg; ?>" data-slug="<?php echo $post->post_name; ?>" data-therapy="<?php echo get_query_var("therapy")[0]; ?>" data-email="<?php echo $therapist_email; ?>"  data-role="<?php echo $role1; ?>" style="<?php if($status == "Busy" || $cstatus == "Busy") {  echo 'border: 1px solid #eb0029 ;background-color:#fff;color:#eb0029;'; } ?>"><i class="fa fa-comments-o <?php if($status == "Busy" || $cstatus == "Busy") { echo "busybtn"; }?>" aria-hidden="true" style="<?php if($status == "Busy" || $cstatus == "Busy") { echo "color: red;";} else { echo "color: #27265f;";}?>"></i>Chat Now </a>
            <?php  
            ...
            <?php }else if(get_sub_field('video_call_button') == 1){?>
              <a class="btnChatnow <?php if($status != "Busy" && $cstatus != "Busy") { echo " book_now_link_oc";} ?> " id="book_now_<?php echo get_the_id(); ?>" data-action="videocall" data-slug="<?php echo $post->post_name; ?>" data-from_status="<?php echo $from_status; ?>" data-therapy="<?php echo get_query_var("therapy")[0]; ?>" style="<?php if($status == "Busy" || $cstatus == "Busy") { echo 'background-color: #80808075 !important;color:#000'; } ?>"><i class="fa fa-calendar <?php if($status == "Busy" || $cstatus == "Busy") { echo "busybtn"; }?>" aria-hidden="true"style="<?php if($status == "Busy" || $cstatus == "Busy") { echo "color: red;";} else { echo "color: #27265f;";}?>"></i>Video Call</a>
            <?php }										  
          }
        }
      endwhile; 
