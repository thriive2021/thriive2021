      ...
      while ( have_rows('button_enabled','option') ) : the_row();
        ...
        if(in_array(get_query_var("therapy")[0], $temparr)){ ?> 
		  ...
		  if(get_query_var('payment_type')){
				...
				?>
					<a class="<?php if($status != "Busy" && $status != "On Call" && $cstatus != "On Chat"){ echo 'btnCallnow'; } else { echo "busybtn";} ?>" style="<?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo 'border: 1px solid #27265f ;background-color:#fff;color:#eb0029;';} else { echo "color: #fff;";}?>" data-target="#0<?php echo $therapist_id;?>"><i class="fa fa-phone <?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo "busybtn"; }?>" aria-hidden="true"  style="<?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo "color: red;";} else { echo "color: #fff;";}?>"></i>Call Now</a>
				<div class="modal fade" style="height: 500px;" data-toggle="modal" id="0<?php echo $therapist_id;?>" data-backdrop="static" data-keyboard="false">
				<div style="margin-top: 240px;margin-left: 20px;margin-right: 20px;" class="modal-dialog <?php if (!is_user_logged_in()){ echo "modal-lg"; } ?>" role="document">
					<div class="modal-content">
						<div class="row">
							<div class="col-sm-12 col-xs-12 text-center" style="margin-bottom: -30px;">
						...
						?>
							<p>Please Confirm To Place Call<p>
							 <a class="btnCallnow" style="padding: 10px;padding-right: 49px;padding-left: 47px;margin: 12px;text-align: center;font-size: 19px;" href="<?php echo site_url();?>/covid-thankyou-page?q=<?php echo $post->post_name; ?>&a=call&t=<?php echo get_query_var("therapy")[0]; ?>">Ok</a>  
						<?php	  
						  }	  ?>  
						</div>
					</div>
           ...
           <?php  
			...
			<a class="<?php if($status != "Busy" && $status != "On Call" && $cstatus != "On Chat"){ echo 'covid_link_oc btnCallnow'; } else { echo "busybtn";} ?>"  data-slug="1<?php echo $therapist_id;?>" data-waiting="<?php echo $sessiondata->waiting; ?>" data-from_status="<?php echo $from_status; ?>" data-therapy="<?php echo get_query_var("therapy")[0]; ?>" data-remaining="0" style="<?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo 'border: 1px solid #27265f ;background-color:#fff;color:#eb0029;';} else { echo "color: #fff;";}?>" data-toggle="modal" ><i class="fa fa-phone <?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo "busybtn"; }?>"  style="<?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo "color: red;";} else { echo "color: #fff;";}?>"></i>Call Now</a>
			<div class="modal fade" style="height: 500px;" id="1<?php echo $therapist_id;?>" data-backdrop="static" data-keyboard="false">
				<div style="margin-top: 240px;margin-left: 20px;margin-right: 20px;" class="modal-dialog <?php if (!is_user_logged_in()){ echo "modal-lg"; } ?>" role="document">
					<div class="modal-content">
						<div class="row">
							<div class="col-sm-12 col-xs-12 text-center" style="margin-bottom: -30px;">
