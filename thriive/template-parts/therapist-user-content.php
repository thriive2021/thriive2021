			...
			if (($role1 == "subscriber" || $role1 =="")){ 
				...
				if($remaining_session > 0){
					...
					if($status != "Busy" && $status != "On Call" && $cstatus != "On Chat"){ 
						...
						$html1	.= 'class= "consult_online_link_oc btnCallnow"'; 
					} else { 
						$html1	.= 'class= " busybtn"';
					} 
					$html1	.= ' id="consult_online_'. get_the_id().'"  data-slug="'.$post->post_name .'" data-waiting="'.$onlinedetails->waiting .'" data-from_status="1" data-therapy="'.get_query_var("therapy")[0].'"  ';
					
					if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { 
					
						$html1	.= 'style= "border: 1px solid #27265f ;background-color:#fff;color:#eb0029;"';
					} else { 
						$html1	.= 'style= "color: #fff;"';
					}
					$html1	.= '><i class="fa fa-phone ';
					if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { 
						$html1	.= "busybtn"; 
					}
