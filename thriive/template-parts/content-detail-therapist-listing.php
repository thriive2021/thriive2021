	...
	<div class="therapiest-listing-wrapper d-flex flex-wrap col-12 p-0">
		...
		<div class="col-sm-6 col-xs-8 col-lg-8 txt-wrap " style="padding:10px 8px 0px 8px !important;">
			...
			if(have_rows('therapy')) {
				...
				$tdtchg = array();
				if(!empty($_GET) && isset($_GET) && $_GET['therapy']){
				...
				<p style="font-family: Segoe UI;line-height: 1.36;" class="m-0 font-14 tt-text">
					<?php if(!empty($_GET) && isset($_GET) && $_GET['therapy']){ ?>
				...
				<p style="font-family: Segoe UI;line-height: 1.36;" class="m-0 font-14">
					...
					<?php foreach ($experience_order as $key => $value){
					    $ord[] = strtotime($value);
					}
				...
				<div itemprop="address" itemscope itemtype="http://schema.org/PostalAddress" class="col-md-8 col-xs-12 font-14">
					...
					if($area) { ?>
						<p style="font-family: Segoe UI;" class=' m-0'>
							...
							echo getTherapistLocation(get_the_id());
						} ?>
						</p>
					<?php } ?>
				</div>
			...
			if(!is_mobile()){
	 		...
	 		$output .= '<div id="start_chat_button_'.$therapist_id.'"><input type="hidden" id="chat_event" value="'.$event.'"><button type="button" class="btn btn-info btn-xs btn-big start_chat" data-img ="'.$url.'" data-fromuserid = "'.$seeker_id.'" data-touserid="'.$therapist_id.'" data-tousername="'.$name.'" data-from_status = "'.$from_status.'" data-to_status = "'.$to_status.'" data-mobile="" data-msg="'.$msg.'" data-email="'.$therapist_email.'"  data-role="'.$role1.'" style="border-radius: 5px; display: none;"><i class="fa fa-comments-o" aria-hidden="true"></i>Chat Now</button></div>';
			$output .= '</div>'; ?> 
			<div class="mt-2" style="font-family: Roboto">
				<?php if (!is_mobile() && ($role1 == 'subscriber' || $role1 =="")){ ?>
					<a style="border-radius:5px; display: none;" href="" id="consult_online_<?php echo get_the_id(); ?>" data-type="1" class="btn btn-primary btn-big consult_online_link"><i class="fa fa-phone" aria-hidden="true"></i> Call Now</a>
		...
		<div class="col-12 mt-1 text-center p-0 m-btn consult_btnGrp" style="font-family: Roboto">	
			<?php if(wp_is_mobile() && ($role1 == 'subscriber' || $role1 =="")){ ?>
				<a href="" id="call_now_<?php echo get_the_id(); ?>" class="btn btn-primary btn-big call_now_link" style="display:none;"><i class="fa fa-phone" aria-hidden="true"></i> Call Now</a>	
				<?php if($role1 == 'subscriber' || $role1 =="")
					echo $output; 
			} ?>
		</div>
