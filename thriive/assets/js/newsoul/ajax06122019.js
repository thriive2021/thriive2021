/*function make_chat_dialog_box(to_user_id, to_user_name, from_user_id = '', user_role = "", img = "") {
    ...
    var modal_content = '<div id="user_dialog_' + to_user_id + '_' + from_user_id + '" class="user_dialog thriive_chat" title="You have chat with ' + to_user_name + '">';
    ...
    modal_content += '<button type="button" name="send_chat"  class="btn send_chat"  onclick="insertData(' + to_user_id + ',' + from_user_id + ');"><i class="fa fa-paper-plane" aria-hidden="true"></i></button><span id= "text_msg" class = "filetype_text" style="display:none;"></span></div></div></div>';
...
$(document).on('click', '.start_chat', function() {
    ...
    if (from_status == 0) {
        ...
        gtag('event', 'click', {
            ...
            event_label: 'chat now clicked - '+eventby,
        });
    ...
    if (from_status != 0) {
        gtag('event', 'click', {
            event_category: 'button',
            event_label: 'chat now initiated - '+eventby,
        });
    }
    if (to_status == 0) {
    ...
    if (!$('[aria-describedby="user_dialog_' + to_user_id + '_' + from_user_id + '"]').is(":visible")) {
        if(from_role == 'subscriber'){
        	var titleStr = "<div class='widget-header widget-header-small' style='padding-left: 21px;'>" + to_user_name + "<button type='button' class='dots_iconz1' onclick='chat_dropDowns();'><span class='dotsA1'></span><span class='dotsA1'></span><span class='dotsA1'></span></button><div class='chat_dropdown_modal' style='display:none;'><ul class='chatdropdwn_lists'><li><button type='button' id= 'del' data-reload = '1' class = 'title_anch' data-to_user = '" + to_user_id + "' data-from_user = '" + from_user_id + "' onclick = 'delete_all(this)'>Delete</button></li><li><a href='https://www.thriive.in/faq-chat' class = 'title_anch' target='_blank' >FAQ</a></li></ul></div><button type='button' class='min_chatBox' data-to_user = '" + to_user_id + "' data-from_user = '" + from_user_id + "' onclick='minim_chatBox(this);'><img src='https://www.thriive.in/wp-content/themes/thriive/assets/images/minim_icon.png' alt='' /></button><button type='button' class='max_chatBox' data-to_user = '" + to_user_id + "' data-from_user = '" + from_user_id + "' onclick='maxim_chatBox(this);'><i class='fa fa-expand' aria-hidden='true'></i></button></div>";
				...
				var titleStr = "<div class='widget-header widget-header-small' style='padding-left: 21px;'>" + to_user_name + "<button type='button' class='dots_iconz1' onclick='chat_dropDowns();'><span class='dotsA1'></span><span class='dotsA1'></span><span class='dotsA1'></span></button><div class='chat_dropdown_modal' style='display:none;'><ul class='chatdropdwn_lists'><li><button type='button' id= 'block' data-reload = '1' class = 'title_anch' data-to_user_name = '"+to_user_name+"' data-to_user = '" + to_user_id + "' data-from_user = '" + from_user_id + "' onclick = 'block_user(this)'>Block</button><button type='button' id='unblock' data-reload = '1' class = 'title_anch' data-to_user_name = '"+to_user_name+"' data-to_user = '" + to_user_id + "' data-from_user = '" + from_user_id + "' onclick = 'unblock_user(this)' style='display:none;'>Unblock</button></li><li><button type='button' id= 'del' data-reload = '1' class = 'title_anch' data-to_user = '" + to_user_id + "' data-from_user = '" + from_user_id + "' onclick = 'delete_all(this)'>Delete</button></li><li><a href='https://www.thriive.in/faq-chat' class = 'title_anch' target='_blank' >FAQ</a></li><li><button type='button' data-toid='" + to_user_id + "' id='complete' class = 'title_anch' style='float:left' onclick='closeModal(" + to_user_id + ',' + from_user_id + ")' >Complete</button></li></ul></div><button type='button' class='min_chatBox' data-to_user = '" + to_user_id + "' data-from_user = '" + from_user_id + "' onclick='minim_chatBox(this);'><img src='https://www.thriive.in/wp-content/themes/thriive/assets/images/minim_icon.png' alt='' /></button><button type='button' class='max_chatBox' data-to_user = '" + to_user_id + "' data-from_user = '" + from_user_id + "' onclick='maxim_chatBox(this);'><i class='fa fa-expand' aria-hidden='true'></i></button></div>";
		}
    ...
    var str = "user_dialog_" + to_user_id + "_" + from_user_id;
    $('[aria-describedby="' + str + '"]').addClass('thriive_chat');
...
function check_is_blocked(to_user_id, from_user_id, to_user_name) {
    $.ajax({
        url: chatajax.ajax_url,
        type: 'POST',
        data: {
            action: 'is_user_blocked', // this is the function in your functions.php that will be triggered
            from_user_id: from_user_id,
            to_user_id: to_user_id,
...
function block_user(e) {
    var to_user = $(e).attr('data-to_user');
    var to_user_name = $(e).attr('data-to_user_name');
    var from_user = $(e).attr('data-from_user');
    var x = confirm("This user will be blocked forever. Are you sure you want to block the user?");
    if (x) {
        $.ajax({
            url: chatajax.ajax_url,
            type: 'POST',
            data: {
                action: 'block_user_by_therapist', // this is the function in your functions.php that will be triggered
                to_user: to_user,
                from_user: from_user,

            },
            success: function(data) {
                $('#chat_user_details').load(" #chat_user_details");
                $('#chat_user_details_mobile').load(" #chat_user_details_mobile");
            var reload = $(e).attr('data-reload');
            ...
            else
            {
                ...
                $('#user_dialog_'+to_user+'_'+from_user+' .emoji_chat_box').css('display','none');
                $('#user_dialog_'+to_user+'_'+from_user).append("<p class='block_msg'>Can't send a message to blocked user "+to_user_name+"</p>");
             update_chat_history_data();
            }
...
function unblock_user(e) {
    ...
    var to_user_name = $(e).attr('data-to_user_name');
    var from_user = $(e).attr('data-from_user');
    var x = confirm("You are trying to unblocked  the block user. Are you sure you want to unblock the user?");
    if (x) {
        $.ajax({
            url: chatajax.ajax_url,
            type: 'POST',
            data: {
                action: 'unblock_user_by_therapist', // this is the function in your functions.php that will be triggered
                to_user: to_user,
                from_user: from_user,
            ...
            else
            {
                ...
                $('#user_dialog_'+to_user+'_'+from_user+' .emoji_chat_box').css('display','block');
                $('#user_dialog_'+to_user+'_'+from_user+' .block_msg').css('display','none');
             update_chat_history_data();
            }
