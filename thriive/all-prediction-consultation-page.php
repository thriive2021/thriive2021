<?php /* Template Name: All online consultation page */ 
...
<style>
    ...
    @media screen and (max-width: 480px) {
		...
		.och1title{color: #333333 !important; text-align:left;margin-top: 18px;margin-bottom: 18px !important;font-weight: 600 !important;}
	...
	<div class="banner-home">
		<div class="container">			
			<select id="select_therapy" class="form-control mb-2 mt-2">
				...
				<h1 class="och1title">Talk to the Best Astrologers in India</h1>
			...
			$getPost = $wpdb->get_results( 
				"SELECT DISTINCT p.ID FROM wp_posts AS p WHERE p.post_type = 'therapist' AND (p.post_status = 'publish' OR p.post_status = 'acf-disabled') AND p.ID IN (49066,56051,57610,43430,28105,53297,24228,24458,37615,44311,53045,33942,60989,54118,60780,51880,56419,28860,26884,14018,24785,22252,26271,56790,49358,48058,46673,46301,40534,24771,27840,29268,37610,29910,7785,8249,39039,26595,29520,40307,28449,10720,23958,54125,46893,35416,41592,27385,39555,35345,36364,46473,6691,10837)"
			);
				...
				<div class="therapist-wrapper section-loop-wrapper row">
					<div class="section_therapies_list custom_therapiest_list col-12 col-sm-8 p-sm-0">
                        ...
                        foreach($tempArr as $t){
                            ...
                            // }
                            setup_postdata( $post );
                            set_query_var('therapy','all-page');
