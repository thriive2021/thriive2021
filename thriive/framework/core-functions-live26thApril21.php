...
function saveOCData($uid,$firstname,$email,$tid,$tfirst_name,$temail,$action,$package,$productinfo,$amount,$status,$txnid,$sendemail,$umobile,$tmobile,$freemins,$coupon,$book_date_time,$tpid){
	...
	if($isexist == 0){
		...
		if($action == 'call'  && $status == 'success'){
			...
			$curl = curl_init();
			$data = array("k_number"=> "+917411782154","agent_number"=> '+91'.$tidmobile,"customer_number"=>'+91'.$uidmobile,"caller_id"=> "+918048166487","additional_params"=>array("object_id"=> "1","user_id"=> "2", "call_log_id" => 1,"timeout"=> ($session_duration)));
			...
			$wpdb->query("UPDATE online_consultation SET tid_accept='".$tid_accept."',uid_accept='".$uid_accept."',connected='".$connected."',waiting=1,email_sms=1,call_id = '".$call_id."',busy_date = '".$busy_date."' WHERE id='".$lastid."'");
		...
		if($sendemail == "yes" && $status == 'success'){
			...
			$alertmsg = $email." connected via ".$action." to ".$tfirst_name." for ".$sessionarr[2]." and for therapy ".$productinfo;
	...
	if($consultrow){
		foreach($consultrow as $cr){
			/* if($cr->call_id != ''){
				...
				$consultrow1 = $wpdb->get_row("SELECT * FROM online_consultation WHERE tid = '". $tid ."' AND DATE(created_date)  = '".$currentdate."' AND TIME(busy_date) BETWEEN '". date('H:i:s') ."' AND '".$currenttime."' AND payment_status = 'success' AND action='call' AND remaining_session > 0");
			//}	
		} 
	}  
	return $consultrow1;
...
function getInprogressConsultation(){ 
	...
	$nobstatusval = ($mval == 'failed')?"selected":"";
	 */
	$sql = "SELECT * FROM `online_consultation` WHERE DATE(created_date) = '".date('Y-m-d')."' ";
	/* if($mval != ""){
		$sql .= " AND payment_status = '". $mval."'";
	}  
	if($sdval != "" && $edval != ""){
