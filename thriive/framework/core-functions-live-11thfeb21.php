...
function saveOCData($uid,$firstname,$email,$tid,$tfirst_name,$temail,$action,$package,$productinfo,$amount,$status,$txnid,$sendemail,$umobile,$tmobile,$freemins,$coupon,$book_date_time,$tpid){
	...
	if($isexist == 0){
		...
		if($status == 'success'){
			$data = array('oc_id'=>$lastid,'uid' => $uid,'tid' => $tid,'therapy_name' => $productinfo ,'action' => $action,'created_date' => $date);
		...
		if(($action == 'book_later' || $action == 'videocall') && $status == 'success'){
			...
			$video_data = array(
				...
				'tid' => $tid,
				'uid' => $uid,
				'book_date' => date("Y-m-d",strtotime($book_date_time)),
				'book_time' => date("H:i:s",strtotime($book_date_time)),
			...
			$tmsg = "Your client ".$uname." has taken ".$sessionarr[2]." for ".$term->name." and the slot is booked on ".$split_datetime[0]." at ".$split_datetime[1].". Please login to your account and start online video call with the client.";
			if($_GET['t'] == "nutritionist"){
				$tmsg .= "Goals :".$goals[$last_user['goals']];
				if($last_user['height'] != ""){
					$tmsg .= " Their BMI index is  ".round($last_user['bmi'],2).". Their Height is  ".$last_user['height'].". Their Weight is  ".$last_user['weight'].". DOB :".$last_user['dob'];
