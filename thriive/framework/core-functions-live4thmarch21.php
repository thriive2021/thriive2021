...
function getTherapistConsultStatus($tid){ 
	...
	$consultrow = $wpdb->get_results("SELECT * FROM online_consultation WHERE tid = '". $tid ."' AND DATE(created_date) = '".$currentdate."' ");
	...
	if($consultrow){
		foreach($consultrow as $cr){
			...
			$consultrow1 = $wpdb->get_var("SELECT COUNT(*) FROM online_consultation WHERE tid = '". $tid ."' AND DATE(created_date)  = '".$currentdate."' AND TIME(busy_date) BETWEEN '". date('H:i:s') ."' AND '".$currenttime."' AND payment_status = 'success'");
		}
	} 
	return $consultrow1;
}
		...
		if($r->action == 'call' || $r->action == 'chat'){
			if($r->action == 'call'){
				...
				if(!empty($chatrow) && count($chatrow) > 1){
					/* foreach($chatrow as $cw){
						if($k == 0){
							$start_chat_time = date("H:i:s",strtotime($cw->chat_time));
						}
						if($k == count($chatrow) -1){
							$end_chat_time = date("H:i:s",strtotime($cw->chat_time));
						}
						$k++;
					} */
