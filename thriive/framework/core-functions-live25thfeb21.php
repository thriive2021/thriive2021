...
function getOnlineConsultation(){
	...
	$html .= "<table class='wp-list-table widefat fixed striped pages'>";
	$html .= "<thead><tr><td>User Name</td><td>User Email</td><td>User Mobile</td><td>Action</td><td>Therapist Name</td><td>Therapist Email</td><td>Therapist Mobile</td><td>Package</td><td>Therapy Name</td><td>Amount</td><td>Call/Chat Status</td><td>Payment Status</td><td>Transaction ID</td><td>Created Date</td></tr></thead>";
	...
	foreach($result as $r){
		...
		$html .= "<td>".$r->amount."</td>";
		if($r->action == 'book_latercall' || $r->action == 'call' || $r->action == 'chat'){
			if($r->action == 'book_latercall' || $r->action == 'call'){
				$condat = getConnectedData($r->tid,$r->uid);
				if(!empty($condat)){
					if(date("Y-m-d",$condat['timestamp']) == date("Y-m-d",strtotime($r->created_date))){
				...
				$chatrow = $wpdb->get_row("SELECT * FROM chat_block_users WHERE from_user_id = '". $r->tid ."' AND to_user_id = '".$r->uid ."'");
				$block_details = json_decode($chatrow->block_details,true);
				if(!empty($block_details)){
					if(date("Y-m-d",strtotime($block_details['date_time'])) == date("Y-m-d",strtotime($r->created_date))){
						$html .= "<td>".date("H:i",strtotime($block_details['date_time']))."</td>";
