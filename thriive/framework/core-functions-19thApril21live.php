...
*/
				...
				sendEmail('accountmanager2@thriive.in',$current_user->user_email.' stage 4 completed', $current_user->user_email.' has submitted the form for verification.');
...
*/
		    ...
		    sendEmail("accountmanager2@thriive.in","Therapist status Changed from $old_status to $new_status","by '$current_user_email' updated the '$post->post_title' --- $post->ID '$new_status'");
		    sendSMS($mobileNo, $smsMsg);
	    } 
	    	...
	    	sendEmail("accountmanager2@thriive.in","$username has submitted the form again to review.","$username missed to filled some important data, the therapist has again submitted the form for review please review the form.");
	    } 
	    ...
	    else if($new_status == 'rejected')
	    {
			...
			sendEmail("accountmanager2@thriive.in","Therapist status Changed to $new_status","by '$current_user_email' updated the '$post->post_title' --- $post->ID '$new_status'");
	    }
	}
...
function saveOCData($uid,$firstname,$email,$tid,$tfirst_name,$temail,$action,$package,$productinfo,$amount,$status,$txnid,$sendemail,$umobile,$tmobile,$freemins,$coupon,$book_date_time,$tpid){
	...
	if($freemins != ""){ 
		...
		if($status == 'success'){
			...
			if($chatblock){
				$block_id = $chatblock->id;
				$wpdb->query("UPDATE chat_block_users SET block_status = 0,complete_status=0,incomplete_status =0 WHERE id = '".$block_id."'");
				
				//$wpdb->query("UPDATE chat_block_users SET block_status = 0,complete_status=0,oc_id ='".$lastid."' WHERE to_user_id = '".$tid."' AND from_user_id = '".$uid."'");
				//$wpdb->query("UPDATE chat_block_users SET block_status = 0,complete_status=0,oc_id ='".$lastid."' WHERE to_user_id = '".$uid."' AND from_user_id = '".$tid."'");
				//$wpdb->query("UPDATE chat_block_users SET block_status = 0,complete_status=0,incomplete_status =0 WHERE to_user_id = '".$tid."' AND from_user_id = '".$uid."'");
				//$wpdb->query("UPDATE chat_block_users SET block_status = 0,complete_status=0,incomplete_status =0 WHERE to_user_id = '".$uid."' AND from_user_id = '".$tid."'");
			} else {
	...
	if($isexist == 0){
		...
		if($action == 'call' && $status == 'success'){
			...
			$tmsg = "Your client ".$uname." has taken ".$sessionarr[2]." and is waiting for a session with you. Please call on the ".$result['masked_number']." to connect with client within the next 2 minutes on thriive.in";
		...
		if(($action == 'chat' || $action == 'book_laterchat') && $status == 'success'){
			date_default_timezone_set('Asia/Kolkata');
			//the code below is moved.  search 'rcpq' to find it (it was not working here somehow)
			/*$chat_data = array(
				'to_user_id' => $tid,
				'from_user_id' => $uid,
				'chat_message' => 'hello',
		...
		if($sendemail == "yes" && $status == 'success'){
			...
			if($action == 'call' && ($productinfo == 'all-page' || $productinfo == 'cosmic-astrology' || $productinfo == 'numerology' || $productinfo == 'tarot-card-reading' || $productinfo == 'angel-reading')){
				...
				$alertmsg = $email." connected via ".$action." to ".$tfirst_name." for ".$package." and for therapy ".$productinfo." on thriive";
			} 
			...
			if($action == 'chat'){
				...
				$umsg = "Your payment for  ".$package." is successful. Please wait for the therapist to confirm the session. You will recieve a message shortly.";
			    ...
			    $msg = "Your payment for  ".$package." is successful. Please wait for the therapist to confirm the session. You will recieve a message shortly.";
				sendEmailWithInvoice($email, $subject, $msg, $uid, 'subscriber', 'oc_invoice.php');
				sendEmailWithInvoice('manishj@thriive.in', $subject, $msg, $uid, 'subscriber', 'oc_invoice.php');
				$alertmsg = $email." connected via ".$action." to ".$tfirst_name." for ".$package." and for therapy ".$productinfo." on thriive";
			}
...
function getInprogressConsultation(){ 
	 ...
	 */
	$sql = "SELECT * FROM `online_consultation` WHERE DATE(created_date) = '".date('Y-m-d')."' AND  payment_status = 'success'";
	/* if($mval != ""){
		$sql .= " AND payment_status = '". $mval."'";
	}  
	if($sdval != "" && $edval != ""){
...
//	$sql .= "  ORDER BY id DESC LIMIT $start,$posts_per_page";  
	...
	foreach($result as $r){
		...
		//echo $post_id->ID;echo "<br/>";
		$status = getTherapistStatus($post_id->ID);
		$consultrow = getTherapistConsultStatus($r->tid,$r-created_date);
		$chatrow =  getTherapistChatStatus($r->tid,$r-created_date);
		if($status == "Available"){ 
			if($consultrow > 0){
				$cstatus = "On Call";
			} else {
		...
		$connected = 0;
		if($cstatus == "On Call" || $cstatus == "On Chat"){ 
			...
			if($r->action == 'call'){
				...
				if(!empty($callrow)){
					$html .= "<td>Connected</td>";$connected = 1;	
				} else {
		...
		//	$html .= "<td>".$r->payment_txnid ."</td>";
			...
			if($r->action == 'call' || $r->action == 'chat' || $r->action == 'videocall'){
				if($r->action == 'call'){
					...
					$chatrow = $wpdb->get_row("SELECT * FROM chat_block_users WHERE (from_user_id = '". $r->uid ."' AND to_user_id = '".$r->tid ."') OR (from_user_id = '". $r->tid ."' AND to_user_id = '".$r->uid ."') AND  DATE(chat_init) = '".date("d-m-Y",strtotime($r->created_date)) ."'");
					$k=0;
					if($chatrow > 0){
						
						$start_chat_time = date("H:i:s",strtotime($chatrow->chat_init));
						$html .= "<td>".$start_chat_time."</td>";
						if($chatrow->complete_status == 1){
							$end_chat_time = date("H:i:s",strtotime($chatrow->complete_time));
			...
			if($r->action == 'call' && $connected == 1){
				$html .= "<td>On Call</td>";
				
			} elseif($r->action == 'chat' && $connected == 1) {
				$html .= "<td>On Chat</td>";
			} else {
				$html .= "<td>--</td>";
			}	
