<?php /* Template Name: Tarot card online consultation page */ 
...
<header class="">
	<div class="banner-home">
		<div class="container">			
			...
			<h1 class="och1title">Talk to the Best Tarot Card Readers in India</h1>
			...
			$getPost = $wpdb->get_results( 
				"SELECT DISTINCT p.ID FROM wp_posts AS p WHERE p.post_type = 'therapist' AND (p.post_status = 'publish' OR p.post_status = 'acf-disabled') AND p.ID IN (49066,56051,57610,43430,28105,53297,24228,24458,23958,27115,27385,28449,35704,37615,44311,53045,33942,60989,54118,60780,51880,56419,49844,49079,51894,50226,38081,49160,48482,30212,15592,2542,37414,23398,44834,46473,6691,10837,24759)"
			);
				...
				<div class="therapist-wrapper section-loop-wrapper row">
					<div class="section_therapies_list therapiest_list col-12 col-sm-8 p-sm-0">
                        ...
                        foreach($tempArr as $t){
                            ...
                            // }
                            setup_postdata( $post );
                            set_query_var('therapy','tarot-card-reading');
