 ...
 */
 ...
 //SMS gateway url
define("SMS_URL","http://sms6.rmlconnect.net:8080/bulksms/bulksms?"."username=THRIIVEGLOBAL&password=y371H2Hv&type=0&dlr=1&source=THRIIV&entityid=1001373488736615240&tempid=&");
...
function fetch_user_chat_history(){
	...
	if($role == 'subscriber'){
    	...
    	$query = " SELECT * FROM chat_message_details  WHERE ((from_user_id = ".$from_user_id."  AND to_user_id = ".$to_user_id.") OR (from_user_id = ".$to_user_id." AND to_user_id = ".$from_user_id.")) AND (delete_status = 0) AND (block_status = 0) ORDER BY chat_message_id ASC ";
    }
...
function fetch_user_therapist(){
	...
	$session_id = $current_user->ID;
	global $wpdb;
    $query = " SELECT * FROM chat_message_details WHERE from_user_id = '".$session_id."' group by to_user_id  ORDER BY chat_time ASC ";
	$result = $wpdb->get_results($query);	
	return($result);
}
...
<a href = "'.$anchor.'" target = "_blank" class="btn btn-info anch_link1 btn btn-primary btn-big btn-transparent connect_with_btn_listing" javascript= "void()">Export</a>
...
<tbody>
						...
						<tr>
							...
							<button type="button" class="btn btn-info btn_link1 view_chat btn btn-primary btn-big btn-transparent connect_with_btn_listing" data-fromuserid = "'.$session_id.'" data-touserid="'.$from_user_id.'" data-tousername="'.$name.'"  data-role="subscriber">View Chat</button>
							<a href = "'.$anchor.'" target = "_blank" class="btn btn-info anch_link1 btn btn-primary btn-big btn-transparent connect_with_btn_listing" javascript= "void()">Export</a></td>
						</tr>
					</tbody>';
	 }
$output .= '</table>				
...
<a href = "'.$anchor.'" target = "_blank" class="btn btn-info btn_link1 anch_link1 btn btn-primary btn-big btn-transparent connect_with_btn_listing" javascript= "void()">Export</a>
...
$query = " SELECT * FROM chat_message_details  WHERE (chat_time >= '".$from_date." 00:00:00' AND chat_time <= '".$to_date." 23:59:59') AND block_status = 0 ORDER BY chat_time DESC ";
	...
	$result = $wpdb->get_results($query);
	//print_r($result);
 $output = '<table class="table table-bordered"><thead><tr><th>User ID</th><th>Message From</th><th>Message To</th><th>Message</th><th>Date Time</th></tr></thead>
					<tbody>';
 foreach($result as $row)
 {
$user_name = ''; 
...
// add_action('wp','test');
// function test(){
// 	echo 'hgfhfvnb';var_dump(is_page('oc-thank-you'));
