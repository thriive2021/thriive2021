		...
		<?php 
			...
			if(count($sdata)!=0){
				...
				if(time()>strtotime($app_time) AND time()<$end_time AND strtotime(date('Y-m-d')) == strtotime($app_date)){
					...
					$vlink = 'https://video.knowlarity.com/room/'.$channel.'?prejoin=false&custom_var='.$oc_id.'&name='.$current_user_name;
					$call_stat = 1;	
				}else{
					$vlink = '';
					$call_stat = 0;
...
<div class="banner-home">
	<div class="container">	
		...
		<div id="therapyres" style="display: none">	
			<section class="topreadingList">	
<?php	
		$sessiondata = $wpdb->get_results("SELECT * FROM online_consultation WHERE uid = '". $current_user->ID ."'  AND remaining_session > 0 AND payment_status = 'success' ORDER BY id DESC ");
		...
		if(!empty($sessiondata)){ 
			...
			foreach($sessiondata as $sd){
				...
				//if(!in_array($val,$duparr)){
					...
					$remaindetails = $wpdb->get_row("SELECT COALESCE(sum(remaining_session), 0) as remaining_session FROM online_consultation WHERE uid = '". $current_user->ID ."'  AND payment_status = 'success'");
