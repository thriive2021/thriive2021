<?php /* Template Name: New Free All online consultation page */ 
...
<select id="select_therapy" class="form-control mb-2 mt-2">
            ...
            <?php if(have_rows('free_therapist_data', 'option')):
                while(have_rows('free_therapist_data', 'option')): the_row();
                    if('all-free-session' == get_sub_field('custom_therapy')){
            ...
            // Loop Start
            if($getPost) { ?>
                <h1 class="och1title">Consult with India's Leading Experts</h1>
                <section class="topreadingList">
                    ...
                    foreach($tempArr as $t){
                        ...
                        if($post->active_for_free_session){
                            setup_postdata( $post );
                            set_query_var('therapy',array('all-free-session'));
