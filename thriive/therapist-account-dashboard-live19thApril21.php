		...
		<div class="row">
			...
			<?php 
				...
				//print_r($therapist_details);
					...
					{
					$is_block_query = $wpdb->get_var("SELECT block_status FROM chat_block_users WHERE to_user_id = $row->from_user_id AND from_user_id = $row->to_user_id AND limit 10");
					$is_blocked = $is_block_query == 1 ? 'Blocked' : 'Active';
					if($is_blocked=='Active'){
						$query = $wpdb->prepare("SELECT complete_status FROM chat_block_users WHERE from_user_id=$row->from_user_id and to_user_id=$row->to_user_id OR from_user_id=$row->to_user_id and to_user_id=$row->from_user_id");
						$com_res = $wpdb->get_results($query);
						if($com_res[0]->complete_status == 0){
...
<div class="container">
	<div class="row">
		<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12"><table class="table" id="chat_user_details_mobile">';
				...
				foreach($result as $row)
				{
					$is_block_query = $wpdb->get_var("SELECT block_status FROM chat_block_users WHERE to_user_id = $row->from_user_id AND from_user_id = $row->to_user_id AND limit 5");
					$is_blocked = $is_block_query == 1 ? 'Blocked' : 'Active';
					if($is_blocked=='Active'){
						$query = $wpdb->prepare("SELECT complete_status FROM chat_block_users WHERE from_user_id=$row->from_user_id and to_user_id=$row->to_user_id OR from_user_id=$row->to_user_id and to_user_id=$row->from_user_id");
						$com_res = $wpdb->get_results($query);
